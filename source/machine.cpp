...
bool Machine::load_binary(const std::string& filename, uint32_t base_addr)
{
    ...
    if (static_cast<size_t>(base_addr) + static_cast<size_t>(size) >
        memory.size())
    {
    ...
    std::cout << "Loaded " << size << " bytes at address 0x" << std::hex
...
void Machine::dump_registers(std::ostream& os) const
{
    ...
    {
        os << "x" << std::dec << std::setw(2) << std::setfill(' ') << i
           << ": 0x" << std::hex << std::setw(8) << std::setfill('0')
           << cpu.get_register(i) << "  " << "x" << (i + 1) << ": 0x"
           << std::setw(8) << std::setfill('0') << cpu.get_register(i + 1)
           << "  " << "x" << (i + 2) << ": 0x" << std::setw(8)
           << std::setfill('0') << cpu.get_register(i + 2) << "  " << "x"
           << (i + 3) << ": 0x" << std::setw(8) << std::setfill('0')
           << cpu.get_register(i + 3) << std::endl;
    }
}
